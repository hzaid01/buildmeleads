...
class Settings:
    ...
    dry_run: bool = _bool("OUTREACH_DRY_RUN", True)
    live_sending_enabled: bool = _bool("LIVE_SENDING_ENABLED", False)
...
def validate_live_configuration(config: Settings = settings) -> list[str]:
    """Return every reason live sending must remain disabled."""
    errors: list[str] = []
    if config.dry_run:
        errors.append("OUTREACH_DRY_RUN must be false")
    if not config.live_sending_enabled:
        errors.append("LIVE_SENDING_ENABLED must be true")
    required = {
